...
struct Employee: EmployeeType {
    ...
    func validate() -> ValidationResult {
        ...
        validator.withKeyPath(\Employee.employeeNumber, name: "employeeNumber", required: true)?.validateMinLength(10).validateMaxLength(10)
        validator.withKeyPath(\Employee.ipv6Address, name: "ipv6Address")?.validateIPV6Address()
        validator.withKeyPath(\Employee.emailAddress, name: "emailAddress")?.validateEmailAddress()
        validator.withKeyPath(\Employee.gender, name: "gender", required: true)
        validator.withKeyPath(\Employee.ipv4Address, name: "ipv4Address")?.validateIPV4Address()
        validator.withKeyPath(\Employee.age, name: "age", required: true)?.validateMinimum(0).validateMaximum(100, exclusive: true)
        validator.withKeyPath(\Employee.hostName, name: "hostName")?.validateHostName()
        validator.withKeyPath(\Employee.someNegativeInteger, name: "someNegativeInteger")?.validateMaximum(0, exclusive: true)
        validator.withKeyPath(\Employee.userName, name: "userName", required: true)?.validateMaxLength(10).validatePattern("^[a-zA-Z0-9]{6,}$")
        validator.withKeyPath(\Employee.married, name: "married", required: true)
        validator.withKeyPath(\Employee.nickNames, name: "nickNames")?.validateMinItems(1).validateMaxItems(5).validateUniqueItems()
        validator.withKeyPath(\Employee.somePositiveInteger, name: "somePositiveInteger")?.validateMinimum(0, exclusive: true)
        validator.withKeyPath(\Employee.petName, name: "petName")?.validateMinLength(6).validatePattern("ing")
        validator.withKeyPath(\Employee.luckyEvenNumber, name: "luckyEvenNumber")?.validateMultipleOf(2)
        validator.withKeyPath(\Employee.personalBest500Meters, name: "personalBest500Meters")?.validateMinimum(5).validateMultipleOf(0.1)
        validator.withKeyPath(\Employee.name, name: "name", required: true)
